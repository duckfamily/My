-- Generated by scripts/build-ui.mjs. Edit src/veyra/*.lua instead.
--[[
VeyraUI 2.0
Roblox desktop UI library. Public API starts with Veyra.new(options).
The generated VeyraUI.lua is standalone; edit src/veyra instead.
]]
...
function Veyra.new(options)
	options = type(options) == "table" and options or {}
	local Library

	local DEFAULT_KEY_SYSTEM = {
		Enabled = true,
		ApiBaseUrl = "https://veyra-key-system.vercel.app",
		Providers = { "workink", "lootlabs" },
		Remember = true,
	}
	...
	local function mergeTable(base, override)
		...
		for key, value in next, override do
			if type(value) == "table" and type(merged[key]) == "table" then
				...
				merged[key] = type(value) == "table" and cloneTable(value) or value
			end
		end
		return merged
	end

	local function resolveKeySystem(value)
		if value == false then
			return false
		end
		if type(value) == "table" then
			return mergeTable(DEFAULT_KEY_SYSTEM, value)
		end
		return cloneTable(DEFAULT_KEY_SYSTEM)
	end
	...
	-- Загрузи PNG из assets/veyra в Roblox и впиши только id.
	...
	local ASSETS = {
		SoftShadow = {
			Id = "139518356096651",
			File = "assets/veyra/soft-shadow.png",
			SliceCenter = Rect.new(32, 32, 96, 96),
		},
		InnerShadow = {
			Id = "75184533556503",
			File = "assets/veyra/inner-shadow.png",
			SliceCenter = Rect.new(24, 24, 72, 72),
		},
		AccentGlow = {
			Id = "114203407815508",
			File = "assets/veyra/accent-glow.png",
			SliceCenter = Rect.new(40, 24, 88, 40),
			SliceScale = 0.5,
		},
		SurfaceNoise = {
			Id = "114070601377247",
			File = "assets/veyra/surface-noise.png",
			TileSize = UDim2.fromOffset(128, 128),
		},
	}

	for name, override in next, type(options.Assets) == "table" and options.Assets or {} do
		if ASSETS[name] then
			if type(override) == "string" or type(override) == "number" then
				ASSETS[name].Id = override
			elseif type(override) == "table" and override.Id ~= nil then
				ASSETS[name].Id = override.Id
			end
		end
	end
	...
	if typeof(getcustomasset) == "function" and typeof(isfile) == "function" then
		for _, def in next, ASSETS do
			if def.Id == "" and def.File then
				for _, candidate in ipairs({ def.File, "VeyraUI/" .. def.File }) do
					local exists = false
					pcall(function()
	...
	local function requestJSON(method, url, body)
		if not HttpService then
			return nil, "HttpService is unavailable."
		end
		if
			type(url) ~= "string"
			or #url > 2048
			or not (url:match("^https://") or url:match("^http://localhost[:/]"))
		then
		...
		if body ~= nil then
			...
			if not ok then
				return nil, "Could not encode the server request."
			end
			encoded = result
		end
	...
	local LOCALES = {
		ru = {
			...
			accessSubtitle = "Выберите сервис и разблокируйте скрипт",
			chooseProvider = "Выберите сервис ключей",
			licenseKey = "Ключ доступа",
			pasteKey = "Вставьте ключ",
			getKey = "Получить ключ",
			verifyKey = "Проверить ключ",
			checking = "Проверка...",
			enterKey = "Сначала введите ключ.",
			chooseProviderHint = "Выберите сервис, получите и введите ключ.",
			accessGranted = "Доступ разрешён.",
			invalidKey = "Ключ недействителен.",
			serverUnavailable = "Не удалось связаться с сервером проверки.",
			keyFooter = "Сервис откроется вне Roblox. Никому не сообщайте пароли от аккаунтов.",
			themeChanged = "Тема изменена",
			configsUnavailable = "Конфиги недоступны",
			configSaved = "Сохранено",
			configLoaded = "Загружено",
			configPartial = "Конфиг загружен частично",
			configExecutorNoFiles = "Исполнитель не даёт доступ к файлам.",
			configLabel = "Конфиг",
			configFailedValues = "Не удалось применить значений:",
			item = "Элемент",
			itemKind = "элемент",
			tab = "Вкладка",
			searchSettings = "Поиск по настройкам",
			closeWindow = "Закрыть окно?",
			closeWindowContent = "Интерфейс будет выгружен. Активные функции скрипта продолжат работу.",
			note = "Заметка",
			textKind = "текст",
			run = "Выполнить",
			doAction = "Выполнить это действие?",
			button = "Кнопка",
			buttonKind = "кнопка",
			toggle = "Тоггл",
			toggleKind = "переключатель",
			slider = "Слайдер",
			sliderKind = "слайдер",
			...
			colorKind = "цвет",
			settings = "Настройки",
			settingsDesc = "Тема, конфигурации и управление интерфейсом",
			appearance = "Внешний вид",
			theme = "Тема",
			themeDesc = "Цветовая схема интерфейса",
			showHide = "Показать / скрыть",
			showHideDesc = "Клавиша переключения окна",
			configurations = "Конфигурации",
			configName = "Имя конфига",
			configNamePlaceholder = "например: pvp",
			save = "Сохранить",
			saveDesc = "Записать текущие значения всех элементов",
			load = "Загрузить",
			loadDesc = "Выберите сохранённую конфигурацию",
			refreshList = "Обновить список",
			refresh = "Обновить",
			dangerZone = "Опасная зона",
			unloadInterface = "Выгрузить интерфейс",
			unloadDesc = "Закрыть окно и освободить память",
			unload = "Выгрузить",
			unloadConfirm = "Интерфейс будет полностью удалён. Продолжить?",
			workinkDescription = "Короткая ссылка и задания для разблокировки",
			lootlabsDescription = "Контент-локер и рекламные предложения",
			player = "игрок",
			noValidator = "Для выбранного сервиса не настроена проверка ключа.",
			linkOpenFailed = "Не удалось открыть ссылку получения ключа.",
			clipboardUnavailable = "Буфер обмена недоступен. Настройте OnGetKey для этого executor-а.",
			verifyingWith = "Проверка через",
			checkingSaved = "Проверка сохранённого ключа",
			destroyed = "VeyraUI уже уничтожен.",
		...
		en = {
			...
			accessSubtitle = "Choose a provider and unlock the script",
			chooseProvider = "Choose key provider",
			...
			chooseProviderHint = "Select a provider, then get and enter your key.",
			accessGranted = "Access granted.",
			invalidKey = "Invalid key.",
			serverUnavailable = "Could not contact the verification service.",
			keyFooter = "The selected provider opens outside Roblox. Never share account passwords.",
			themeChanged = "Theme changed",
			configsUnavailable = "Configs unavailable",
			configSaved = "Saved",
			configLoaded = "Loaded",
			configPartial = "Config loaded partially",
			configExecutorNoFiles = "The executor does not provide file access.",
			configLabel = "Config",
			configFailedValues = "Values that could not be applied:",
			item = "Item",
			itemKind = "item",
			tab = "Tab",
			searchSettings = "Search settings",
			closeWindow = "Close window?",
			closeWindowContent = "The interface will unload. Active script functions will continue running.",
			note = "Note",
			textKind = "text",
			run = "Run",
			doAction = "Run this action?",
			button = "Button",
			buttonKind = "button",
			toggle = "Toggle",
			toggleKind = "toggle",
			...
			colorKind = "color",
			settings = "Settings",
			settingsDesc = "Theme, configuration, and interface management",
			appearance = "Appearance",
			theme = "Theme",
			themeDesc = "Interface color scheme",
			showHide = "Show / hide",
			showHideDesc = "Window toggle key",
			configurations = "Configurations",
			configName = "Config name",
			configNamePlaceholder = "for example: pvp",
			save = "Save",
			saveDesc = "Write the current value of every control",
			load = "Load",
			loadDesc = "Choose a saved configuration",
			refreshList = "Refresh list",
			refresh = "Refresh",
			dangerZone = "Danger zone",
			unloadInterface = "Unload interface",
			unloadDesc = "Close the window and release memory",
			unload = "Unload",
			unloadConfirm = "The interface will be completely removed. Continue?",
			workinkDescription = "Short link and unlock tasks",
			lootlabsDescription = "Content locker and offers",
			player = "player",
			noValidator = "No key validator is configured for the selected provider.",
			linkOpenFailed = "Could not open the key link.",
			clipboardUnavailable = "Clipboard is unavailable. Configure OnGetKey for this executor.",
			verifyingWith = "Verifying with",
			checkingSaved = "Checking saved key for",
			destroyed = "VeyraUI has been destroyed.",
			authorizationCancelled = "Authorization was cancelled.",
		},
	}

	local function localeCode(value)
		value = type(value) == "string" and value:lower() or "ru"
		return LOCALES[value] and value or "ru"
	end

	local function tr(key, fallback)
		local locale = Library and Library.Locale or localeCode(options.Locale)
		local dictionary = LOCALES[locale] or LOCALES.ru
		return dictionary[key] or LOCALES.en[key] or fallback or key
	end
	...
	local Theme = {}
	for k, v in next, THEMES.Veyra do
		Theme[k] = v
	end
	...
	local GradientRegistry = setmetatable({}, { __mode = "k" })

	local function removeThemeEntries(inst)
		ThemeRegistry[inst] = nil
		GradientRegistry[inst] = nil
	end

	local function register(inst, prop, key)
		local bindings = ThemeRegistry[inst]
		if not bindings then
			bindings = {}
			ThemeRegistry[inst] = bindings
			pcall(function()
				inst.Destroying:Connect(function()
					removeThemeEntries(inst)
				end)
			end)
		end
		bindings[prop] = key
	...
	local function registerGradient(gradient, keyA, keyB)
		GradientRegistry[gradient] = { a = keyA, b = keyB }
		pcall(function()
			gradient.Destroying:Connect(function()
				removeThemeEntries(gradient)
			end)
		end)
		pcall(function()
	...
	do
		...
		local ok = pcall(function()
			return Font.new(family, Enum.FontWeight.Medium)
		end)
		if ok then
	...
	local effectOptions = type(options.Effects) == "table" and options.Effects or {}
	local Effects = {
		Shadow = effectOptions.Shadow ~= false,
		InnerShadow = effectOptions.InnerShadow ~= false,
		Glow = effectOptions.Glow ~= false,
		Noise = effectOptions.Noise ~= false,
	}

	local function tween(inst, info, props)
		local active = ActiveTweens[inst]
		if not active then
			active = {}
			ActiveTweens[inst] = active
		end
		local cancelled = {}
		for prop in next, props do
			local previous = active[prop]
	...
	local function new(class, props, children)
		...
		for k, v in next, props or {} do
			if k == "Parent" then
				parent = v
			elseif k == "Theme" then
				for prop, key in next, v do
					register(inst, prop, key)
				end
	...
	local function assetLayer(parent, name, props)
		...
		if def.SliceCenter then
			...
			if def.SliceScale then
				base.SliceScale = def.SliceScale
			end
	...
	local function innerShadow(parent, z, transparency)
		if not Effects.InnerShadow then
			return nil
		end
	...
	local function snapGuiPosition(inst)
		...
		inst.Position = UDim2.new(position.X.Scale, position.X.Offset + dx, position.Y.Scale, position.Y.Offset + dy)
	end
	...
	local function safeCall(fn, ...)
		...
		task.spawn(function()
			...
			if not ok then
				warn("[VeyraUI] ошибка в коллбэке: " .. tostring(err))
			end
			PendingCallbacks = math.max(0, PendingCallbacks - 1)
		end)
	end

	local function waitForCallbacks(timeout)
		local deadline = os.clock() + finiteNumber(timeout, 5)
		while PendingCallbacks > 0 and os.clock() < deadline do
			task.wait()
		end
	...
	Library = {
		Version = Veyra.Version,
		Windows = {},
		SearchIndex = {}, -- для командной палитры
		ThemeName = type(options.Theme) == "string" and options.Theme or "Veyra",
		Locale = localeCode(options.Locale),
		Alive = true,
		Authorized = false,
		KeySystem = resolveKeySystem(options.KeySystem),
		_config = options,
		_conns = {},
		_render = {},
	}

	if not THEMES[Library.ThemeName] then
		Library.ThemeName = "Veyra"
	end
	for key, value in next, THEMES[Library.ThemeName] do
		Theme[key] = value
	end

	local function optionFlag(opts)
		local flag = opts and (opts.Id or opts.ID)
	...
	local function unregisterOption(flag, obj)
		...
		if owner and owner._options and owner._options[flag] == obj then
			...
			if owner._values then
				owner._values[flag] = nil
			end
	...
	local function bindCleanup(inst, fn)
		...
		local function cleanup()
			...
			if not ok then
				warn("[VeyraUI] очистка элемента: " .. tostring(err))
			end
		end
		pcall(function()
			inst.Destroying:Connect(cleanup)
		end)
		return cleanup
	end
	...
	local function track(conn)
		if conn then
			table.insert(Library._conns, conn)
		end
		return conn
	end
	...
	local function onRender(fn)
		...
		return function()
			for i, f in ipairs(Library._render) do
				if f == fn then
					table.remove(Library._render, i)
					break
				end
			end
		end
	...
	local function fire(kind, input, processed)
		...
		for _, fn in ipairs(table.clone(bucket)) do
			if table.find(bucket, fn) then
				...
				if not ok then
					warn("[VeyraUI] ввод: " .. tostring(err))
				end
			end
		end
	end
	...
	local function draggable(handle, target, onMove, canStart)
		handle.InputBegan:Connect(function(input)
			if not isPressStart(input) or (canStart and not canStart()) then
				return
			end
			...
			activeDrag = function(i)
				...
				target.Position = UDim2.new(
					...
					startPosition.X.Offset + adjusted.X,
					startPosition.Y.Scale,
					startPosition.Y.Offset + adjusted.Y
				)
				if onMove then
					onMove()
				end
	...
	local Gui = new("ScreenGui", {
		Name = "Veyra_" .. tostring(math.random(100000, 999999)),
		ResetOnSpawn = false,
	...
	local function guardPopup()
		popupGuard = true
	end

	onInput("Began", function(input)
		if not openPopup or not isPressStart(input) then
			return
		end
	...
	local ToastLayer = new("Frame", {
		...
		new("UISizeConstraint", { MaxSize = Vector2.new(360, 600) }),
	})
	...
	function Library:Notify(opts)
		...
		local title = asText(opts.Title, tr("notification"))
		...
		tween(card, EASE, { BackgroundTransparency = 0, Size = UDim2.new(1, 0, 0, toastHeight) })
		...
		local function dismiss()
			...
			if bodyLbl then
				tween(bodyLbl, EASE_FAST, { TextTransparency = 1 })
			end
			local s = card:FindFirstChildOfClass("UIStroke")
			if s then
				tween(s, EASE_FAST, { Transparency = 1 })
			end
	...
	function Library:Dialog(opts)
		...
		local dialogWidth = round(math.clamp(viewport().X - 32, 300, 380), 0)
		...
		local box = new("Frame", {
			...
			AnchorPoint = Vector2.new(0.5, 0.5),
			Position = UDim2.fromScale(0.5, 0.5),
			Size = UDim2.fromOffset(dialogWidth, 0),
			ZIndex = 951,
		...
		label({
			...
			Font = FONT_B,
			TextSize = 15,
			Text = asText(opts.Title, tr("confirm")),
			ZIndex = 953,
		})
		...
		local function close()
			...
			tween(box, EASE_FAST, { Size = UDim2.fromOffset(dialogWidth, 0) })
			shade.Active = false
		...
		tween(box, EASE, { Size = UDim2.fromOffset(dialogWidth, dialogHeight) })
	...
	-- Строка внутри явно переданной секции.
	local function makeCard(_, opts, slotWidthOverride, heightOverride)
		local section = opts._VeyraSection
		if type(section) ~= "table" or not section.card or not section.card.Parent then
			error("VeyraUI components must be created through section:Add...()")
		end
		local desc = opts.Desc ~= nil and tostring(opts.Desc) or nil
		local hasDesc = desc ~= nil and desc ~= ""
		local height =
			math.max(32, finiteNumber(heightOverride ~= nil and heightOverride or opts.Height, hasDesc and 62 or 52))
		local slotWidth =
			math.max(0, finiteNumber(slotWidthOverride ~= nil and slotWidthOverride or opts.SlotWidth, 140))
		...
		local title = label({
			...
			Font = FONT_M,
			TextSize = 13,
			Text = asText(opts.Text ~= nil and opts.Text or opts.Name, tr("item")),
			TextTruncate = Enum.TextTruncate.AtEnd,
		})
		...
		pcall(function()
			row:SetAttribute(
				"VeyraSearch",
				table.concat({
					asText(opts.Id or opts.ID, ""),
					asText(opts.Desc, ""),
					type(opts.Keywords) == "table" and table.concat(opts.Keywords, " ") or asText(opts.Keywords, ""),
				}, " ")
			)
		end)
		...
		return row, slot, title, description
	end
	...
	local KEY_PROVIDER_META = {
		WorkInk = {
			Name = "Work.ink",
			DescriptionKey = "workinkDescription",
			Icon = "link",
		},
		LootLabs = {
			Name = "LootLabs",
			DescriptionKey = "lootlabsDescription",
			Icon = "gift",
		},
	}
	...
	local function keyCacheFile(window, config, provider)
		...
		return "VeyraUI/keys/" .. keyCacheStem(owner) .. "_" .. provider.Id:lower() .. ".json"
	end

	local function ensureKeyCacheFolder()
		if not FS.mkdir or not FS.isdir then
			return
		end
		for _, folder in ipairs({ "VeyraUI", "VeyraUI/keys" }) do
			local checked, exists = pcall(FS.isdir, folder)
			if not checked or not exists then
				pcall(FS.mkdir, folder)
			end
		end
	end
	...
	local function readKeyCache(window, config, provider)
		...
		if not read or type(raw) ~= "string" or #raw > 16384 then
			return nil
		end
		local decoded, data = pcall(HttpService.JSONDecode, HttpService, raw)
		if not decoded or type(data) ~= "table" then
		...
		if data.version ~= 2 or data.provider ~= provider.Id or type(data.token) ~= "string" or #data.token > 4096 then
			return nil
		end
		if data.permanent ~= true and (type(data.expiresAt) ~= "number" or data.expiresAt <= os.time()) then
			if FS.delete then
	...
	local function writeKeyCache(window, config, provider, metadata)
		if config.Remember == false or provider.Remember == false or not HAS_FS or not HttpService then
			return false
		end
		if type(metadata) ~= "table" or type(metadata.token) ~= "string" then
			return false
		end
		local permanent = metadata.permanent == true
		local expiresAt = tonumber(metadata.expiresAt)
		if not permanent and (not expiresAt or expiresAt <= os.time()) then
			return false
		end
		...
		local encoded, raw = pcall(HttpService.JSONEncode, HttpService, cacheData)
		if not encoded then
			return false
		end
	...
	local function clearKeyCache(window, config, provider)
		if not FS.delete or not FS.isfile then
			return
		end
		local path = keyCacheFile(window, config, provider)
		local checked, exists = pcall(FS.isfile, path)
	...
	local function normalizeKeyProviders(config)
		...
		local providers = {}
		local serverValue = config.ApiBaseUrl or config.Server
		local server
		if type(serverValue) == "string" then
			server = serverValue:match("^%s*(.-)%s*$")
			if server then
				server = server:gsub("/+$", "")
			end
		end
		...
		for _, value in ipairs(source) do
			local id = tostring(value):lower()
			if id == "workink" or id == "work.ink" then
				enabled.WorkInk = true
			end
			if id == "lootlabs" or id == "loot labs" then
				enabled.LootLabs = true
			end
		end

		local function add(id)
			local meta = KEY_PROVIDER_META[id]
			if #source > 0 and not enabled[id] then
				return
			end
			...
			table.insert(providers, {
				...
				Description = asText(raw.Description, tr(meta.DescriptionKey)),
	...
	local function validateKey(provider, key, cached)
		local validator = provider.Validate
		if type(validator) == "function" then
			local ok, valid, message, token = pcall(validator, key, provider.Id, cached == true)
			if not ok then
				warn("[VeyraUI] Key validation failed: " .. tostring(valid))
				return false, tr("serverUnavailable")
			end
			if type(valid) == "table" then
				local result = valid
				local success = result.Success == true or result.Valid == true
				return success,
					asText(result.Message, success and tr("accessGranted") or tr("invalidKey")),
					{
						token = result.Token or result.Key or key,
						permanent = result.Permanent == true,
						expiresAt = result.ExpiresAt,
					}
			end
			local success = valid == true
			local metadata = type(token) == "table" and token or { token = token or key }
			return success, asText(message, success and tr("accessGranted") or tr("invalidKey")), metadata
		end

		return false, tr("noValidator")
	end
	...
	local function runKeySystem(window, options, main)
		...
		if #providers == 0 then
			warn("[VeyraUI] KeySystem has no enabled providers.")
			return false
		end
		...
		local gateViewport = viewport()
		local gateWidth = round(math.clamp(gateViewport.X - 24, 320, 500), 0)
		local gateHeight = round(math.clamp(gateViewport.Y - 24, 400, 450), 0)

		local function restoreShell()
			if shellRestored or not main.Parent then
				return
			end
			shellRestored = true
		...
		main.Size = UDim2.fromOffset(gateWidth, gateHeight)
		main.BackgroundTransparency = 1
		if shellClip then
			shellClip.Visible = false
		end
		if shellStroke then
		...
		label({
			...
			Font = FONT_B,
			TextSize = 18,
			Text = asText(config.Title, tr("accessRequired")),
			ZIndex = 304,
		})
		label({
			Parent = card,
			...
			Font = FONT_M,
			TextSize = 12,
			Text = asText(config.Subtitle, tr("accessSubtitle")),
			TextTruncate = Enum.TextTruncate.AtEnd,
			ZIndex = 304,
			Theme = { TextColor3 = "Muted" },
		})
		...
		label({
			...
			Font = FONT_SB,
			TextSize = 12,
			Text = tr("chooseProvider"),
			ZIndex = 304,
			Theme = { TextColor3 = "SubText" },
		})

		local providerButtons = {}
		local providerGap = 10
		local providerWidth = (#providers == 1) and (gateWidth - 48) or math.floor((gateWidth - 48 - providerGap) / 2)
		for i, provider in ipairs(providers) do
			local button = new("TextButton", {
				Parent = card,
				Position = UDim2.fromOffset(24 + (i - 1) * (providerWidth + providerGap), 110),
		...
		local input = new("TextBox", {
			...
			TextSize = 13,
			Text = "",
			PlaceholderText = asText(config.Placeholder, tr("pasteKey")),
			TextXAlignment = Enum.TextXAlignment.Left,
			ZIndex = 306,
		...
		local actionWidth = math.floor((gateWidth - 48 - providerGap) / 2)
		local function actionButton(text, x, primary)
			return new("TextButton", {
				Parent = card,
				Position = UDim2.fromOffset(x, 326),
				Size = UDim2.fromOffset(actionWidth, 42),
				AutoButtonColor = false,
				Font = FONT_SB,
				TextSize = 13,
				Text = text,
		...
		local getButton = actionButton(asText(config.GetKeyText, tr("getKey")), 24, false)
		local verifyButton =
			actionButton(asText(config.VerifyText, tr("verifyKey")), 24 + actionWidth + providerGap, true)
		...
		label({
			...
			Font = FONT_M,
			TextSize = 11,
			Text = asText(config.Footer, tr("keyFooter")),
			TextWrapped = true,
			TextYAlignment = Enum.TextYAlignment.Top,
			ZIndex = 304,
			Theme = { TextColor3 = "Muted" },
		...
		for provider, refs in next, providerButtons do
			refs.Button.MouseButton1Click:Connect(function()
				if busy or resolved or selected == provider then
					return
				end
				selected = provider
				input.Text = ""
		...
		local function resolveURL(provider)
			...
			if type(source) == "function" then
				...
				if not ok then
					warn("[VeyraUI] GetKeyURL failed: " .. tostring(result))
			...
			if not url or url == "" then
				return nil, "No key link is configured for " .. provider.Name .. "."
			end
			return url
		end

		getButton.MouseButton1Click:Connect(function()
			if busy or resolved then
				return
			end
			...
			if type(selected.OnGetKey) == "function" then
				...
				if not ok then
					warn("[VeyraUI] OnGetKey failed: " .. tostring(handled))
					setStatus(tr("linkOpenFailed"), "Bad")
					return
				end
			...
			if copyToClipboard(url) then
				...
				setStatus(tr("clipboardUnavailable"), "Bad")
			end
		end)

		local function unlock(provider, metadata, message)
			if resolved or not main.Parent then
				return
			end
			resolved = true
			busy = false
			...
			writeKeyCache(window, config, provider, metadata)
		...
		local function tryKey(provider, key, cached)
			...
			verifyButton.Text = tr("checking")
			if not cached then
				setStatus(tr("verifyingWith") .. " " .. provider.Name .. "...", "Accent")
			end

			task.spawn(function()
				local valid, message, metadata = validateKey(provider, key, cached)
				if resolved or not gate.Parent then
					return
				end
				busy = false
				verifyButton.Active = true
				verifyButton.Text = asText(config.VerifyText, tr("verifyKey"))
				if valid then
					metadata = type(metadata) == "table" and metadata or { token = key }
					metadata.token = asText(metadata.token, key)
					unlock(provider, metadata, message)
				else
					if cached then
						clearKeyCache(window, config, provider)
					end
		...
		task.defer(function()
			for _, provider in ipairs(providers) do
				...
				if cached then
					selected = provider
					refreshProviders()
					setStatus(tr("checkingSaved") .. " " .. provider.Name .. "...", "Accent")
					tryKey(provider, cached.token, true)
					while busy and not resolved and main.Parent do
						task.wait()
					end
				end
			end
			if not resolved and main.Parent then
				setStatus(tr("chooseProviderHint"), "Muted")
			end
		end)
	...
	function Library:Authorize()
		if not self.Alive then
			return false, tr("destroyed")
		end
		if self.Authorized then
			return true
		end
		local config = self.KeySystem
		if config == nil or config == false or (type(config) == "table" and config.Enabled == false) then
		...
		local owner = {
			Title = asText(self._config.Title, "Veyra"),
			ConfigName = self._config.Config or self._config.ConfigKey,
			Clip = nil,
			KeySystemLocked = true,
		}
		local ok = runKeySystem(owner, config, host)
		if host.Parent then
			host:Destroy()
		end
		self.Authorized = ok == true
		if self.Authorized then
			return true
		end
		return false, tr("authorizationCancelled")
	end
	...
	function Window:GetValue(id)
		return self._values and self._values[tostring(id)]
	end

	function Window:GetControl(id)
		return self._options and self._options[tostring(id)]
	end

	function Window:SetValue(id, value, silent)
		local control = self:GetControl(id)
		if not control or type(control.Set) ~= "function" then
			return false
		end
		return control:Set(value, silent == true)
	end

	function Window:_ApplyChrome(minimized)
		self.Sidebar.Visible = not minimized
		self._sidebarToggle.Visible = not minimized
		self.Pages.Visible = not minimized
		self._grip.Visible = not minimized and not self.Maximized
		if minimized then
			self.Content.Position = UDim2.new()
			self.Content.Size = UDim2.fromScale(1, 1)
		else
			local width = self.SidebarOpen and self.SidebarWidth or 0
			self.Content.Position = UDim2.fromOffset(width, 0)
			self.Content.Size = UDim2.new(1, -width, 1, 0)
		end
	end

	function Window:_SetMinIcon(name)
		for _, child in ipairs(self._btnMinBox:GetChildren()) do
			child:Destroy()
		end
		if Icons[name] then
			Icons[name](self._btnMinBox, "Muted", 4)
		end
	end

	function Window:SetSidebarOpen(state)
		if not Library.Alive or not self.Frame.Parent or self.Minimized then
			return false
		end
		state = state == true
		if self.SidebarOpen == state then
			return true
		end
		self.SidebarOpen = state
		closePopup()
		local width = self.SidebarWidth
		tween(self.Sidebar, EASE, { Size = UDim2.new(0, state and width or 0, 1, 0) })
		tween(self.Content, EASE, {
			Position = UDim2.fromOffset(state and width or 0, 0),
			Size = UDim2.new(1, state and -width or 0, 1, 0),
		})
		tween(self._sidebarToggle, EASE, {
			Position = UDim2.fromOffset(state and (width - 46) or 14, 20),
		})
		tween(self._sidebarArrow, EASE, { Rotation = state and 90 or -90 })
		return true
	end

	function Window:SetSidebarWidth(width)
		if not Library.Alive or not self.Frame.Parent then
			return false
		end
		width = round(math.clamp(finiteNumber(width, self.SidebarWidth), 210, 340), 0)
		self.SidebarWidth = width
		local currentViewport = viewport()
		self.MinSize = Vector2.new(
			math.min(math.max(self._minimumWindowWidth, width + 280), math.max(320, currentViewport.X - 32)),
			math.min(self.MinSize.Y, math.max(180, currentViewport.Y - 32))
		)
		self.Sidebar.Size = UDim2.new(0, self.SidebarOpen and width or 0, 1, 0)
		self.Content.Position = UDim2.fromOffset(self.SidebarOpen and width or 0, 0)
		self.Content.Size = UDim2.new(1, self.SidebarOpen and -width or 0, 1, 0)
		self._sidebarToggle.Position = UDim2.fromOffset(self.SidebarOpen and (width - 46) or 14, 20)
		return true
	end
	...
	function Window:SetMinimized(state)
		...
		self._minimizeToken += 1
		local token = self._minimizeToken
		closePopup()
		if self._hideResults then
			self._hideResults()
		end
		if state then
			if not self.Hidden then
				local absolute = self.Frame.AbsoluteSize
				if absolute.X > 0 and absolute.Y > 0 then
					self._savedSize = UDim2.fromOffset(round(absolute.X, 0), round(absolute.Y, 0))
				end
				tween(self.Frame, EASE, { Size = UDim2.fromOffset(380, 90) })
			end
			self:_ApplyChrome(true)
			self:_SetMinIcon("expand")
		else
			if self.Hidden then
				self:_ApplyChrome(false)
			else
				tween(self.Frame, EASE, { Size = self._savedSize })
				task.delay(0.22, function()
					if
						token == self._minimizeToken
						and not self.Minimized
						and not self.Hidden
						and self.Frame.Parent
					then
						self:_ApplyChrome(false)
						snapGuiPosition(self.Frame)
					end
				end)
			end
			self:_SetMinIcon("minimize")
		end
		return true
	end
	...
	function Window:SetVisible(state)
		...
		if state then
			...
			self:_ApplyChrome(self.Minimized)
			local targetSize = self.Minimized and UDim2.fromOffset(380, 90) or self._savedSize
			tween(self.Frame, EASE, { Size = targetSize })
			task.delay(0.22, function()
				if token == self._visibilityToken and not self.Hidden and self.Frame.Parent then
					snapGuiPosition(self.Frame)
				end
			end)
		else
			...
			local width = self.Frame.Size.X
			tween(self.Frame, EASE_FAST, { Size = UDim2.new(width.Scale, width.Offset, 0, 0) })
			task.delay(0.14, function()
				if token == self._visibilityToken and self.Hidden and self.Frame.Parent then
					self.Frame.Visible = false
				end
			end)
		end
		if self.FloatButton then
			self.FloatButton.Visible = not state
		end
		refreshBlur()
		return true
	end
	...
	function Window:Notify(options)
		return self.App:Notify(options)
	end
	...
	function Window:SetScale(scale)
		...
		self.Scale = scale
		tween(self._scaleObject, EASE_FAST, { Scale = scale })
		return true
	end
	...
	function Library:CreateWindow(opts)
		if not self.Alive then
			return nil, tr("destroyed")
		end
		if self._window and self._window.Frame and self._window.Frame.Parent then
			return nil, "This Veyra app already owns a window."
		end
		if not self.Authorized then
			local authorized, reason = self:Authorize()
			if not authorized then
				return nil, reason or tr("authorizationCancelled")
			end
		end
		opts = type(opts) == "table" and opts or {}
		local app = self
		...
		self.Title = asText(opts.Title, "Veyra")
		...
		self._minimumWindowWidth = minimumWindowWidth
		local sidebarWidth = round(math.clamp(finiteNumber(opts.SidebarWidth, 288), 210, 340), 0)
		self.SidebarWidth = sidebarWidth
		self.MinSize = Vector2.new(
			math.min(math.max(minimumWindowWidth, sidebarWidth + 280), math.max(320, vp.X - 32)),
			math.min(self.MinSize.Y, math.max(180, vp.Y - 32))
		)
		local requested = typeof(opts.Size) == "UDim2" and opts.Size or UDim2.fromOffset(1180, 730)
		local size = UDim2.fromOffset(
			round(
				math.clamp(
					requested.X.Offset + requested.X.Scale * vp.X,
					self.MinSize.X,
					math.max(self.MinSize.X, vp.X - 32)
				),
				0
			),
			round(
				math.clamp(
					requested.Y.Offset + requested.Y.Scale * vp.Y,
					self.MinSize.Y,
					math.max(self.MinSize.Y, vp.Y - 32)
				),
				0
			)
		)
		self._savedSize = size
		...
		-- Упрощённый знак Veyra: две острые ленты без Unicode-глифов и картинок.
		new("Frame", {
			Parent = logo,
			AnchorPoint = Vector2.new(0.5, 0.5),
			Position = UDim2.new(0.37, 0, 0.49, 0),
		...
		new("Frame", {
			Parent = logo,
			AnchorPoint = Vector2.new(0.5, 0.5),
			Position = UDim2.new(0.63, 0, 0.49, 0),
			Size = UDim2.fromOffset(5, 28),
			Rotation = 24,
			ZIndex = 3,
			Theme = { BackgroundColor3 = "Accent" },
		...
		self._sidebarToggle = SidebarToggle
		self._sidebarArrow = sidebarArrow
		...
		local SearchInput = new("TextBox", {
			...
			TextSize = 13,
			Text = "",
			PlaceholderText = asText(opts.SearchPlaceholder, tr("searchSettings")),
			ClearTextOnFocus = false,
			TextXAlignment = Enum.TextXAlignment.Left,
			Theme = { TextColor3 = "Text", PlaceholderColor3 = "Muted" },
		})
		...
		local displayUser = LocalPlayer and LocalPlayer.DisplayName or asText(opts.UserText, tr("player"))
		...
		Grip.InputBegan:Connect(function(input)
			if not isPressStart(input) or self.Minimized or self.Hidden or self.Maximized then
				return
			end
			local origin = input.Position
			...
			activeDrag = function(i)
				...
				Main.Size = UDim2.fromOffset(
					round(
						math.clamp(startSize.X + d.X, self.MinSize.X, math.max(self.MinSize.X, currentViewport.X - 32)),
						0
					),
					round(
						math.clamp(startSize.Y + d.Y, self.MinSize.Y, math.max(self.MinSize.Y, currentViewport.Y - 32)),
						0
					)
				)
		...
		draggable(TopBar, Main, closePopup, function()
			return not self.Maximized
		end)
		draggable(Brand, Main, closePopup, function()
			return not self.Maximized
		end)
		...
		local restoreSize = self._savedSize
		local restorePosition = Main.Position
		BtnMax.MouseButton1Click:Connect(function()
			if self.Minimized or self.Hidden then
				return
			end
			if self.Maximized then
				self.Maximized = false
				Grip.Visible = true
				self._savedSize = restoreSize
				tween(Main, EASE, { Size = restoreSize, Position = restorePosition })
				task.delay(0.24, function()
					if not self.Maximized and Main.Parent then
						snapGuiPosition(Main)
					end
				end)
			else
				self.Maximized = true
				Grip.Visible = false
				restoreSize = UDim2.fromOffset(round(Main.AbsoluteSize.X, 0), round(Main.AbsoluteSize.Y, 0))
				restorePosition = Main.Position
				local vp = viewport()
				local target = UDim2.fromOffset(round(math.max(320, vp.X - 72), 0), round(math.max(180, vp.Y - 72), 0))
				self._savedSize = target
				tween(Main, EASE, { Size = target, Position = UDim2.fromScale(0.5, 0.5) })
				task.delay(0.24, function()
					if self.Maximized and Main.Parent then
						snapGuiPosition(Main)
					end
				end)
			end
		...
		BtnClose.MouseButton1Click:Connect(function()
			Library:Dialog({
				Title = tr("closeWindow") .. " " .. self.Title,
				Content = tr("closeWindowContent"),
				Buttons = {
					{
						Text = tr("close"),
						Primary = true,
		...
		local stopWindowInput = onInput("Began", function(input, processed)
			...
			if self.KeySystemLocked then
				return
			end
			local ctrlK = input.KeyCode == Enum.KeyCode.K
				and (UserInput:IsKeyDown(Enum.KeyCode.LeftControl) or UserInput:IsKeyDown(Enum.KeyCode.RightControl))
			if ctrlK then
			...
			if
				not processed
				and self.ToggleKey
				and (input.KeyCode == self.ToggleKey or input.UserInputType == self.ToggleKey)
			then
				self:Toggle()
			end
		end)
		...
		if UserInput.TouchEnabled then
			...
			float.InputBegan:Connect(function(input)
				if not isPressStart(input) then
					return
				end
				floatDragged = false
				local origin = input.Position
		...
		-- CreateWindow запускает Authorize автоматически и показывает окно только после успеха.
		...
		local function activateSearchEntry(entry)
			if not entry then
				return
			end
			...
			task.defer(function()
				...
				if not page or not page.Parent or not entry.card or not entry.card.Parent then
					return
				end
				local targetY = entry.card.AbsolutePosition.Y - page.AbsolutePosition.Y + page.CanvasPosition.Y - 16
		...
		local function paintSearchSelection()
			for index, result in ipairs(resultItems) do
				tween(result.button, EASE_FAST, {
					BackgroundTransparency = index == selectedResult and 0 or 1,
				})
			end
		end
		...
		local function runSearch(query)
			...
			for i = #Library.SearchIndex, 1, -1 do
				...
				if
					not entry.card
					or not entry.card.Parent
					or not entry.tab
					or not entry.tab.Page
					or not entry.tab.Page.Parent
				then
			...
			for _, entry in ipairs(Library.SearchIndex) do
				if entry.window == self and entry.searchName:find(query, 1, true) then
					...
					table.insert(resultItems, { button = item, entry = entry })
				end
			end
			...
			Results.Position = overlayPosition(SearchBox.AbsolutePosition.X, SearchBox.AbsolutePosition.Y + 38)
		...
		SearchInput.FocusLost:Connect(function()
			task.delay(0.15, function()
				if SearchInput.Parent and not SearchInput:IsFocused() then
					hideResults()
				end
			end)
		end)
		local stopSearchInput = onInput("Began", function(input)
			if not Results.Visible or not SearchInput:IsFocused() or #resultItems == 0 then
				return
			end
			if input.KeyCode == Enum.KeyCode.Down then
				selectedResult = selectedResult % #resultItems + 1
				paintSearchSelection()
			elseif input.KeyCode == Enum.KeyCode.Up then
				selectedResult = ((selectedResult - 2) % #resultItems) + 1
				paintSearchSelection()
			elseif input.KeyCode == Enum.KeyCode.Return or input.KeyCode == Enum.KeyCode.KeypadEnter then
				if selectedResult == 0 then
					selectedResult = 1
				end
				activateSearchEntry(resultItems[selectedResult].entry)
			end
		end)
		...
		local function cleanupWindow()
			...
			if Library._window == self then
				Library._window = nil
			end
			if Library.Alive then
				task.defer(refreshBlur)
			end
		end
	...
	function Window:SelectTab(tab)
		...
		self.ActiveTab = tab
		if self.TabTitle then
			self.TabTitle.Text = tab.Name
		end
		if self.TabDesc then
			self.TabDesc.Text = tab.Desc or ""
		end
	...
	function Window:AddTab(opts)
		if not Library.Alive or not self.Pages or not self.Pages.Parent then
			return nil
		end
		opts = type(opts) == "table" and opts or {}
		local tab = setmetatable({}, Tab)
		tab.Name = asText(opts.Name ~= nil and opts.Name or opts.Text, tr("tab"))
		...
		local function updateColumns()
			if not columnsHost.Parent then
				return
			end
			local sideBySide = tab._columnCount > 1 and tab.Page.AbsoluteSize.X >= 760
			columnsLayout.FillDirection = sideBySide and Enum.FillDirection.Horizontal or Enum.FillDirection.Vertical
			for _, column in ipairs(tab._columns) do
				column.Size = sideBySide and UDim2.new(1 / tab._columnCount, -5, 0, 0) or UDim2.new(1, 0, 0, 0)
			end
		end
	...
	local function index(tab, name, kind, card)
		local entry = {
			window = tab.Window,
			tab = tab,
			name = asText(name, tr("item")),
			kind = asText(kind, tr("itemKind")),
			card = card,
		}
		entry.searchName = normalizeSearch(table.concat({
			entry.name,
			entry.kind,
			tab.Name or "",
			tab.Desc or "",
			card:GetAttribute("VeyraSearch") or "",
		}, " "))
	...
	function Tab:AddSection(opts)
		...
		return setmetatable({ _tab = self, _raw = section }, Section)
	end

	--──────────────────────────── Разделитель ────────────────────────────
	-- Разделитель внутри явно выбранной секции.
	function Tab:_Divider(opts)
		if not tabAlive(self) then
			return nil
		end
		local section = opts and opts._VeyraSection
		if type(section) ~= "table" or not section.card or not section.card.Parent then
			return nil
		end
	...
	function Tab:_Paragraph(opts)
		...
		local title = label({
			...
			Font = FONT_M,
			TextSize = 13,
			Text = asText(opts.Text ~= nil and opts.Text or opts.Title, tr("note")),
			LayoutOrder = 1,
		})
		local body = label({
			Parent = row,
		...
		local searchEntry = index(self, title.Text, tr("textKind"), row)
		return {
			Set = function(_, t, c)
				if not row.Parent then
					return false
	...
	function Tab:_Button(opts)
		...
		local btn = new("TextButton", {
			...
			Font = FONT_SB,
			TextSize = 13,
			Text = asText(opts.ButtonText, tr("run")),
		...
		btn.MouseButton1Click:Connect(function()
			...
			task.delay(0.09, function()
				if btn.Parent then
					tween(btn, EASE_FAST, { Size = UDim2.fromOffset(controlWidth, 30) })
				end
			end)
			if opts.Confirm then
				Library:Dialog({
					Title = asText(opts.Text, tr("confirm")),
					Content = opts.Confirm == true and tr("doAction") or tostring(opts.Confirm),
					Buttons = {
						{ Text = tr("yes"), Primary = true, Callback = opts.Callback },
						{ Text = tr("cancel") },
					},
				})
			else
		...
		index(self, asText(opts.Text, tr("button")), tr("buttonKind"), card)
		return {
			Destroy = function()
				if card.Parent then
					card:Destroy()
	...
	function Tab:_Toggle(opts)
		...
		function obj:Set(value, silent)
			...
			if flag and obj.Window then
				obj.Window._values[flag] = state
			end
			if not silent then
		...
		index(self, asText(opts.Text, tr("toggle")), tr("toggleKind"), card)
		return obj
	end

	function Section:SetText(text)
		if not self._raw.holder.Parent then
			return false
		end
		if self._raw.header then
			self._raw.header.Text = asText(text, "")
		end
		return true
	end

	function Section:Destroy()
		if self._raw.holder.Parent then
			self._raw.holder:Destroy()
		end
	end

	local function copyOptions(opts)
		local copy = {}
		for key, value in next, type(opts) == "table" and opts or {} do
			copy[key] = value
		end
		return copy
	end

	local function decorateControl(control, listeners)
		if not control then
			return nil
		end
		...
		function control:SetVisible(state)
			if not self.Card or not self.Card.Parent then
				return false
			end
			self.Card.Visible = state ~= false
			return true
		end
		...
		function control:SetEnabled(state)
			...
			self.Card.Active = self._enabled
			for _, child in ipairs(self.Card:GetDescendants()) do
				if child:IsA("GuiButton") or child:IsA("TextBox") then
					child.Active = self._enabled
					pcall(function()
						child.TextEditable = self._enabled
					end)
				end
			end
			self.Card:SetAttribute("VeyraEnabled", self._enabled)
			return true
		end
		...
		function control:SetText(text)
			...
			local title = self.Card:FindFirstChild("Title", true)
			if not title or not title:IsA("TextLabel") then
				return false
			end
			title.Text = asText(text, "")
			for _, entry in ipairs(Library.SearchIndex) do
				if entry.card == self.Card then
					entry.name = title.Text
					entry.searchName = normalizeSearch(
						table.concat({ title.Text, entry.kind, entry.tab.Name or "", entry.tab.Desc or "" }, " ")
					)
				end
			end
			return true
		...
		function control:SetDescription(text)
			...
			local description = self.Card:FindFirstChild("Description", true)
			if not description or not description:IsA("TextLabel") then
				return false
			end
			description.Text = asText(text, "")
			for _, entry in ipairs(Library.SearchIndex) do
				if entry.card == self.Card then
					entry.searchName = normalizeSearch(
						table.concat(
							{ entry.name, entry.kind, entry.tab.Name or "", entry.tab.Desc or "", description.Text },
							" "
						)
					)
				end
	...
	function Section:_create(method, opts)
		if not self._raw.holder.Parent then
			return nil
		end
		local listeners = {}
		local prepared = copyOptions(opts)
		local id = optionFlag(prepared)
		local window = self._tab.Window
		if id and window._options[id] then
			warn("[VeyraUI] duplicate control Id in one window: " .. id)
			return nil
		end
		local callback = prepared.Callback
		prepared._VeyraSection = self._raw
		prepared.Callback = function(...)
			if callback then
				safeCall(callback, ...)
			end
		...
		local ok, control = pcall(method, self._tab, prepared)
		if not ok then
			warn("[VeyraUI] component creation failed: " .. tostring(control))
			return nil
		end
		control = decorateControl(control, listeners)
		if id and not window._options[id] then
			window._options[id] = control
			control.Window = window
			local destroy = control.Destroy
			function control:Destroy()
				unregisterOption(id, self)
				return destroy(self)
			end
		end
		return control
	...
	function Section:AddParagraph(opts)
		return self:_create(Tab._Paragraph, opts)
	end
	function Section:AddButton(opts)
		return self:_create(Tab._Button, opts)
	end
	function Section:AddToggle(opts)
		return self:_create(Tab._Toggle, opts)
	end
	function Section:AddSlider(opts)
		return self:_create(Tab._Slider, opts)
	end
	function Section:AddDropdown(opts)
		return self:_create(Tab._Dropdown, opts)
	end
	function Section:AddInput(opts)
		return self:_create(Tab._Input, opts)
	end
	function Section:AddKeybind(opts)
		return self:_create(Tab._Keybind, opts)
	end
	function Section:AddColorPicker(opts)
		return self:_create(Tab._ColorPicker, opts)
	end
	function Section:AddDivider()
		return self:_create(Tab._Divider, {})
	end

	function Section:AddNumberInput(opts)
		opts = copyOptions(opts)
		local minimum = finiteNumber(opts.Min, -math.huge)
		local maximum = finiteNumber(opts.Max, math.huge)
		if maximum < minimum then
			minimum, maximum = maximum, minimum
		end
		local step = math.abs(finiteNumber(opts.Step, 1))
		if step == 0 then
			step = 1
		end
		local decimals = math.clamp(math.floor(finiteNumber(opts.Decimals, 0)), 0, 6)
		local callback = opts.Callback
		local function normalize(value)
			value = math.clamp(finiteNumber(value, finiteNumber(opts.Default, 0)), minimum, maximum)
			value = round(value / step, 0) * step
			return round(value, decimals)
		end
		opts.Default = string.format("%." .. decimals .. "f", normalize(opts.Default))
		opts.Numeric = true
		...
		local control = self:AddInput(opts)
		if not control then
			return nil
		end
		local rawGet, rawSet, rawOnChanged = control.Get, control.Set, control.OnChanged
		function control:Get()
			return normalize(rawGet(self))
		end
		function control:Set(value, silent)
			local number = normalize(value)
			return rawSet(self, string.format("%." .. decimals .. "f", number), silent)
		end
		function control:OnChanged(listener)
			return rawOnChanged(self, function(value)
				listener(normalize(value))
			end)
		end
		return control
	end

	function Tab:_Segmented(opts)
		local flag = optionFlag(opts)
		local values = type(opts.Options) == "table" and opts.Options or {}
		local state = opts.Default
		if state == nil then
			state = values[1]
		end
		local width = math.max(160, finiteNumber(opts.SlotWidth, math.max(1, #values) * 72))
		local card, slot = makeCard(self, opts, width)
		local buttons = {}
		local obj: any = { Card = card }
		local cleanup
		new("UIListLayout", {
			Parent = slot,
			FillDirection = Enum.FillDirection.Horizontal,
		...
		local function paint()
			for value, button in next, buttons do
				local active = value == state
				setThemeKey(button, "BackgroundColor3", active and "Accent" or "Inset")
				button.BackgroundColor3 = Theme[active and "Accent" or "Inset"]
			end
		end

		function obj:Set(value, silent)
			if not card.Parent or table.find(values, value) == nil then
				return false
			end
			state = value
			if flag and obj.Window then
				obj.Window._values[flag] = value
			end
			paint()
			if not silent then
		...
		for order, value in ipairs(values) do
			local button = new("TextButton", {
				Parent = slot,
				Size = UDim2.new(1 / math.max(1, #values), -2, 0, 30),
				LayoutOrder = order,
				AutoButtonColor = false,
				Font = FONT_M,
				TextSize = 12,
		...
		obj:Set(state, true)
		registerOption(flag, obj, state, self.Window)
		index(self, asText(opts.Text, "Segmented"), "segmented", card)
		return obj
	end

	function Section:AddSegmented(opts)
		return self:_create(Tab._Segmented, opts)
	end

	function Tab:_Progress(opts)
		local flag = optionFlag(opts)
		local minimum = finiteNumber(opts.Min, 0)
		local maximum = finiteNumber(opts.Max, 100)
		if maximum <= minimum then
			maximum = minimum + 1
		end
		local state = math.clamp(finiteNumber(opts.Default, minimum), minimum, maximum)
		local card, slot = makeCard(self, opts, 190)
		...
		local fill = new("Frame", {
			...
			Theme = { BackgroundColor3 = "Accent" },
		}, { corner(4), registerGradient(new("UIGradient", {}), "Accent", "Accent2") })
		local obj: any = { Card = card }
		local cleanup
		function obj:Set(value, silent)
			if not card.Parent then
			...
			state = math.clamp(finiteNumber(value, state), minimum, maximum)
			if flag and obj.Window then
				obj.Window._values[flag] = state
			end
			tween(fill, EASE, { Size = UDim2.fromScale((state - minimum) / (maximum - minimum), 1) })
			if not silent then
				safeCall(opts.Callback, state)
			end
			return true
		...
		obj:Set(state, true)
		registerOption(flag, obj, state, self.Window)
		index(self, asText(opts.Text, "Progress"), "progress", card)
		return obj
	end

	function Section:AddProgress(opts)
		return self:_create(Tab._Progress, opts)
	end

	--────────────────────────────── Слайдер ──────────────────────────────
	function Tab:_Slider(opts)
		if not tabAlive(self) then
			return nil
		end
		opts = type(opts) == "table" and opts or {}
		...
		index(self, asText(opts.Text, tr("slider")), tr("sliderKind"), card)
		return obj
	end

	--──────────────────────────── Выпадающий список ────────────────────────────
	function Tab:_Dropdown(opts)
		if not tabAlive(self) then
			return nil
		end
		opts = type(opts) == "table" and opts or {}
		...
		local placeholder = asText(opts.Placeholder, tr("notSelected"))
		...
		local function refreshDisplay()
			if multi then
				...
				elseif #selected <= 2 then
					...
					display.Text = tr("selected") .. " " .. #selected
					setThemeKey(display, "TextColor3", "Text")
					display.TextColor3 = Theme.Text
				end
			else
		...
		local function close()
			...
			task.delay(0.13, function()
				if token ~= popupToken or isOpen then
					return
				end
				if menu.Parent then
					menu.Visible = false
				end
				if menuShadow and menuShadow.Parent then
		...
		local function applyFilter()
			...
			for _, row in ipairs(rows) do
				row.frame.Visible = query == "" or normalizeSearch(row.value):find(query, 1, true) ~= nil
			end
		end
		...
		function obj:Set(value, silent)
			...
			if multi then
				...
				if type(value) == "table" then
					for _, item in ipairs(value) do
						...
						if item ~= nil and not hasOption(item) then
							return false
						end
			...
			if flag then
				if obj.Window then
					obj.Window._values[flag] = multi and table.clone(selected) or selected
				end
			end
			if not silent then
				safeCall(opts.Callback, multi and table.clone(selected) or selected)
			end
			return true
		end

		function obj:Get()
			return multi and table.clone(selected) or selected
		end
		...
		if default == nil and multi then
			default = {}
		end
		obj:Set(default, true)
		registerOption(flag, obj, multi and table.clone(selected) or selected, self.Window)
		index(self, asText(opts.Text, tr("list")), tr("listKind"), card)
		return obj
	end

	--──────────────────────────── Поле ввода ────────────────────────────
	function Tab:_Input(opts)
		if not tabAlive(self) then
			return nil
		end
		opts = type(opts) == "table" and opts or {}
		...
		local input = new("TextBox", {
			...
			PlaceholderText = asText(opts.Placeholder, tr("enter")),
			ClearTextOnFocus = false,
			TextXAlignment = Enum.TextXAlignment.Left,
			Theme = { TextColor3 = "Text", PlaceholderColor3 = "Muted" },
		})
		...
		input.FocusLost:Connect(function(enter)
			...
			if flag and obj.Window then
				obj.Window._values[flag] = input.Text
			end
			if enter or not opts.OnEnter then
				safeCall(opts.Callback, input.Text, enter)
			end
		end)
		...
		function obj:Set(v, silent)
			...
			if not silent then
				safeCall(opts.Callback, input.Text, false)
			end
			return true
		end
		function obj:Get()
		...
		index(self, asText(opts.Text, tr("input")), tr("inputKind"), card)
		return obj
	end

	--────────────────────────────── Кейбинд ──────────────────────────────
	function Tab:_Keybind(opts)
		if not tabAlive(self) then
			return nil
		end
		opts = type(opts) == "table" and opts or {}
		...
		local function isConfiguredModifier(keyCode)
			for _, modifier in ipairs(modifiers) do
				if modifierMatchesKey(modifier, keyCode) then
					return true
				end
			end
			return false
		...
		local function modifiersHeld()
			for _, modifier in ipairs(modifiers) do
				...
				elseif name == "shift" then
					held = UserInput:IsKeyDown(Enum.KeyCode.LeftShift) or UserInput:IsKeyDown(Enum.KeyCode.RightShift)
				elseif name == "alt" then
					held = UserInput:IsKeyDown(Enum.KeyCode.LeftAlt) or UserInput:IsKeyDown(Enum.KeyCode.RightAlt)
		...
		function obj:Set(key, silent)
			...
			if typeof(key) == "EnumItem" then
				...
				local validKeyCode = keyCode and key ~= Enum.KeyCode.None
				local validMouseButton = inputType
					and (
						key == Enum.UserInputType.MouseButton1
						or key == Enum.UserInputType.MouseButton2
						or key == Enum.UserInputType.MouseButton3
					)
				if not validKeyCode and not validMouseButton then
					key = nil
				end
			else
				key = nil
			end
			if supplied ~= nil and key == nil then
				return false
			end
			entry.key = key
			if comboMode then
				keyCapLabel.Text = key and key.Name or tr("none")
				btn.Text = ""
			else
		...
		obj.Card = card
		cleanup = bindCleanup(card, function()
			if captureTarget == captureFn then
				captureTarget = nil
			end
		...
		entry.fire = function()
			if not modifiersHeld() then
				return
			end
			if opts.OnPress then
				safeCall(opts.OnPress, entry.key)
			end
			if opts.Mode == "toggle" and opts.Toggle ~= nil then
				local targetFlag = tostring(opts.Toggle)
				local target = self.Window and self.Window._options[targetFlag]
		...
		index(self, asText(opts.Text, tr("key")), tr("keyKind"), card)
		return obj
	end

	--──────────────────────────── Выбор цвета ────────────────────────────
	function Tab:_ColorPicker(opts)
		if not tabAlive(self) then
			return nil
		end
		opts = type(opts) == "table" and opts or {}
		...
		function obj:Set(c, silent)
			...
			if flag and obj.Window then
				obj.Window._values[flag] = color
			end
			if not silent then
				safeCall(opts.Callback, color)
			end
			return true
		end
		...
		local function close()
			...
			task.delay(0.13, function()
				...
				if popShadow and popShadow.Parent then
					popShadow.Visible = false
				end
			end)
		end
		...
		local function place()
			...
			if y + 182 > vp.Y - 10 then
				y = swatchBtn.AbsolutePosition.Y - 188
			end
		...
		cleanup = bindCleanup(card, function()
			close()
			popupToken += 1
			if activeDrag == svDrag or activeDrag == hueDrag then
				activeDrag = nil
			end
			unregisterOption(flag, obj)
			if pop.Parent then
		...
		index(self, asText(opts.Text, tr("color")), tr("colorKind"), card)
		return obj
	end
	...
	local function safeFilePart(value, fallback)
		...
		if
			...
			or upperStem == "AUX"
			or upperStem == "NUL"
			or upperStem:match("^COM[1-9]$")
			or upperStem:match("^LPT[1-9]$")
		then
			text = "_" .. text
		end
	...
	function Window:ConfigFolder()
		return "VeyraUI/" .. safeFilePart(self.ConfigName or game.PlaceId, tostring(game.PlaceId))
	end

	local function ensureFolder(path)
		if not FS.isdir or not FS.mkdir then
			return
		end
		local parts = {}
	...
	function Window:SaveConfig(name)
		...
		if not HAS_FS then
			Library:Notify({
				Title = tr("configsUnavailable"),
				Content = tr("configExecutorNoFiles"),
				Type = "warn",
			})
			return false
		end
		...
		local ok, encoded = pcall(function()
			...
			for _, flag in ipairs(self._optionOrder) do
				...
				if element and element.Get then
					...
					if not got then
						error("failed to read option: " .. tostring(flag))
					end
					values[tostring(flag)] = encodeValue(value)
				end
			end
			return HttpService:JSONEncode({
				schema = 2,
				veyraVersion = Library.Version,
				savedAt = os.time(),
				values = values,
			})
		end)
		...
		if written then
			Library:Notify({
				Title = tr("configSaved"),
				Content = tr("configLabel") .. " «" .. name .. "»",
				Type = "success",
			})
		end
		return written
	...
	function Window:LoadConfig(name)
		...
		if not ok or type(raw) ~= "string" or #raw > 1024 * 1024 then
			return false
		end
		...
		local values
		if data.schema == 2 and type(data.values) == "table" then
			values = data.values
		elseif data.schema == nil then
			values = data -- one-time import of the v1 flat format
		else
			return false
		end
		...
		for _, key in ipairs(self._optionOrder) do
			...
			if value ~= nil then
				...
				if not decoded or invalidTagged then
					return false
				end
				decodedValues[key] = { value = result, taggedNil = taggedNil }
			end
		end

		-- Apply only after the entire document has been decoded successfully.
		local failed = 0
		for _, key in ipairs(self._optionOrder) do
			local decoded = decodedValues[key]
			if decoded then
				local element = self._options[key]
				if element and element.Set then
					local applyValue = decoded.value
					if decoded.taggedNil then
						applyValue = nil
					end
					local called, applied = pcall(element.Set, element, applyValue)
		...
		if failed > 0 then
			Library:Notify({
				Title = tr("configPartial"),
				Content = tr("configFailedValues") .. " " .. failed,
				Type = "warn",
			})
			return false
		end
		Library:Notify({
			Title = tr("configLoaded"),
			Content = tr("configLabel") .. " «" .. name .. "»",
			Type = "success",
		})
		return true
	...
	function Window:ListConfigs()
		...
		if not HAS_FS or not FS.list then
			return out
		end
		local folder = self:ConfigFolder()
		if FS.isdir then
	...
	function Window:DeleteConfig(name)
		...
		if not HAS_FS or not FS.delete then
			return false
		end
		name = safeFilePart(name, "default")
		local path = self:ConfigFolder() .. "/" .. name .. ".json"
	...
	function Window:AddSettingsTab(opts)
		...
		local appearance = tab:AddSection({ Text = tr("appearance") })
		...
		local configs = tab:AddSection({ Text = tr("configurations") })

		local configDropdown
		local nameInput = configs:AddInput({
			Text = tr("configName"),
			Placeholder = tr("configNamePlaceholder"),
			Default = "default",
		})
		...
		configDropdown = configs:AddDropdown({
			Text = tr("load"),
			Desc = tr("loadDesc"),
		...
		configs:AddButton({
			Text = tr("refreshList"),
			ButtonText = tr("refresh"),
			Callback = function()
				configDropdown:Refresh(self:ListConfigs())
			end,
		})

		local danger = tab:AddSection({ Text = tr("dangerZone") })

		danger:AddButton({
			Text = tr("unloadInterface"),
			Desc = tr("unloadDesc"),
			ButtonText = tr("unload"),
			Confirm = tr("unloadConfirm"),
			Callback = function()
				Library:Destroy()
			end,
		})
	...
	function Library:SetAccent(color)
		if not self.Alive or typeof(color) ~= "Color3" then
			return false
		end
		self._accentOverride = color
		Theme.Accent = color
		Theme.Accent2 = color:Lerp(Color3.new(1, 1, 1), 0.22)

		for inst, bindings in next, ThemeRegistry do
			if inst.Parent then
				for prop, key in next, bindings do
					if key == "Accent" or key == "Accent2" then
						tween(inst, EASE, { [prop] = Theme[key] })
					end
				end
			else
				ThemeRegistry[inst] = nil
		...
		for inst, entry in next, GradientRegistry do
			if not inst.Parent then
				GradientRegistry[inst] = nil
			elseif entry.a == "Accent" or entry.a == "Accent2" or entry.b == "Accent" or entry.b == "Accent2" then
				local ok = pcall(function()
					inst.Color = ColorSequence.new(Theme[entry.a], Theme[entry.b])
				end)
				if not ok then
					GradientRegistry[inst] = nil
				end
	...
	function Library:SetTheme(name, silent)
		if not self.Alive then
			return false
		end
		...
		if self._accentOverride then
			Theme.Accent = self._accentOverride
			Theme.Accent2 = self._accentOverride:Lerp(Color3.new(1, 1, 1), 0.22)
		end
		...
		for inst, entry in next, GradientRegistry do
			...
			if not inst.Parent or not colorA or not colorB then
				GradientRegistry[inst] = nil
			else
				local ok = pcall(function()
					inst.Color = ColorSequence.new(colorA, colorB)
				end)
				if not ok then
					GradientRegistry[inst] = nil
				end
		...
		if not silent then
			self:Notify({ Title = tr("themeChanged"), Content = name, Type = "success" })
		end
		return true
	end

	function Library:RegisterTheme(name, palette)
		if not self.Alive or type(name) ~= "string" or name == "" or type(palette) ~= "table" then
		...
		for _, key in ipairs(required) do
			if typeof(palette[key]) ~= "Color3" then
				return false, "Missing Color3 theme key: " .. key
			end
			copy[key] = palette[key]
		end
		THEMES[name] = copy
		return true
	end
	...
	for name, palette in next, type(options.Themes) == "table" and options.Themes or {} do
		Library:RegisterTheme(name, palette)
	end
	if type(options.Theme) == "string" and THEMES[options.Theme] then
		Library:SetTheme(options.Theme, true)
	end
	if typeof(options.Accent) == "Color3" then
		Library:SetAccent(options.Accent)
	end
	Library:SetAnimations(options.Animations ~= false)
