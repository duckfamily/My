...
local function updateRaidTimer(dt)
    ...
    if doubleXp then extras[#extras + 1] = "2XP" end
...
local function rebuildLoot()
    ...
    if loots ~= lootWatchRoot then
        lootWatchRoot = loots
        conns[#conns + 1] = loots.DescendantAdded:Connect(function() lootDirty = true end)
        conns[#conns + 1] = loots.DescendantRemoving:Connect(function() lootDirty = true end)
    end
